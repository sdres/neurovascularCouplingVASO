...
for sub in ['sub-01']:
    ...
    runs = sorted(glob.glob(f'{root}/{sub}/{ses}/func/sub-01_ses-01_task-stimulation_run-001_part-mag_*.nii.gz'))
    ...
    os.system(f'{afniPath}/3dTcat -prefix {outFolder}/{base}_combined.nii  {outFolder}/{base}_moco_bold.nii {outFolder}/{base}_moco_cbv.nii -overwrite')
    # Calculating T1w image in EPI space for each run
    os.system(f'{afniPath}/3dTstat -cvarinv -overwrite -prefix {outFolder}/{base}_T1w.nii {outFolder}/{base}_combined.nii')
    # Running biasfieldcorrection
    os.system(f'{antsPath}/N4BiasFieldCorrection -d 3 -i {outFolder}/{base}_T1w.nii -o {outFolder}/{base}_T1w_N4Corrected.nii')
